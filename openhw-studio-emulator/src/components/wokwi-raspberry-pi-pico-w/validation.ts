  ...
  rules: [
    {
      ...
      check: (component: any, graph: Map<string, string[]>, validator: any) => {
        ...
        if (vbusVolts > 5.5) {
          return `FRIED [Pico W ${component.id}]: ${vbusVolts}V applied to VBUS. Max expected is 5V USB input.`;
        }

        if (v3Volts > 3.6) {
          return `FRIED [Pico W ${component.id}]: ${v3Volts}V applied to 3V3 rail.`;
        }
