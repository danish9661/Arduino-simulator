...
export const validation: { rules: ComponentValidationRule[] } = {
    rules: [
        {
            id: 'pca9685-i2c-interface-check',
            name: 'I2C Interface Check',
            severity: 'warn',
            priority: 10,
            description: 'Warn when the PCA9685 power, ground, or I2C pins are disconnected.',
            check: (component: any, graph: Map<string, string[]>, validator: any) => {
                ...
                if (vcc && validator.getNeighbors(`${component.id}.${vcc}`).length === 0) {
                    issues.push(createValidationIssue({
                        ruleId: 'pca9685-i2c-interface-check',
                        severity: 'warn',
                        message: `⚠️ [${component.type} ${component.id}] Logic Power (VCC) is not connected.`,
