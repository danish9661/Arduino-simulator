...
```
✗ ledPin = 12 (mismatch) → -2 pts
✗ Missing second delay() → -5 pts
(other variables OK)
...
```
✗ ledPin = 12                    → -2 pts
✗ Initial ledState = HIGH        → -2 pts
✗ Missing delay in loop          → -5 pts
✗ Inverted conditional           → -3 pts
...
If drift 400-500ms:
  ...
  Penalty = 3 × 1 = 3 points ✗
  
If drift > 500ms:
  Penalty = 3-9 points (up to 3x multiplier) ✗
```
...
Student events (captured at 8x speed):
  PinChange(13, HIGH, 25ms)   S0 → matches T0 (drift +3ms ✓)
  PinChange(13, LOW,  519ms)  S1 → matches T1 (drift +2ms ✓)
  PinChange(13, HIGH, 1004ms) S2 → matches T2 (drift +3ms ✓)
...
If drift ≤ 400ms BUT value differs:
  Value mismatch: -2 points ✗
  
If drift > 400ms:
  Time drift penalty: 1-3 points ✗
```
...
Matches:
  S0 → T0 (status match, voltage match, drift +2ms ✓)
  S1 → T1 (status match, voltage match, drift +2ms ✓)
...
Timeline Matching Algorithm:
  For each teacher event T at index i:
    ...
    If S found within 400ms drift:
      Match at best (lowest drift) index → +weight points ✓
    
    If no match within window:
      Unmatched event → -penalty points ✗

Example:
  Teacher events: [E0, E1, E2, E3, E4]
  Student events: [E0', E1', E2', E3', E4', Extra1, Extra2]
  
  E0 matches E0' at exact index 0     ✓
  E1 matches E1' at exact index 1     ✓
  E2 matches E2' at exact index 2     ✓
  E3 matches E3' at exact index 3     ✓
  E4 matches E4' at exact index 4     ✓
...
Pin Changes (weight 4):
  T0↔S0: drift +3ms ✓ → +4 pts
  T1↔S1: drift +3ms ✓ → +4 pts
  T2↔S2: drift +3ms ✓ → +4 pts
  T3↔S3: drift +3ms ✓ → +4 pts
  T4↔S4: drift +3ms ✓ → +4 pts
  T5 ignored (beyond cutoff)
  Subtotal: 20 pts

Component States (weight 2):
  12 teacher events → 12 student events match
  All within drift tolerance ✓
  12 × 2 = 24 pts
...
✓ Compiles
✓ Executes
✗ Logic doesn't match (e.g., delays swapped)
→ Code_Score = 50
```
...
SPATIAL SCORE:
  ✓ Pin 13 correct
  ✓ LED correctly placed
  ✓ Resistor correctly placed
...
LOGIC SCORE:
  ✓ ledPin = 13 (correct)
  ✓ delayMs = 500 (correct)
  ✓ digitalWrite sequence correct
  ✓ delay sequence correct
  → Logic_Score = 100
...
Pin Change Matches:
  T0-T8: drift +3-50ms ✓ (8 matches)
  T9-T15: drift +450-550ms ✗ (7 mismatches! All exceed 400ms threshold)
...
**Use this guide to**:
1. ✅ Understand where points are earned/lost
2. ✅ Predict your student's score before grading
3. ✅ Identify root causes of low scores
4. ✅ Provide targeted feedback to students
