...
### Problem
- **Bundle 1 (4x)**: Captured 108–114 events ✅
- **Bundle 2 (8x)**: Captured only 37 events (66% loss) ❌
- **Bundle 3 (1x)**: Captured 114 events ✅
...
- **Bundle 1 (4x)**: Drift evolves from +252ms → +116ms (improves over time) ✅
- **Bundle 2 (8x)**: Drift evolves from -1ms → +1096ms (worsens severely) ❌
- **Bundle 3 (1x)**: Drift stays within ±33ms (stable) ✅
...
### Finding
Bundle 1 has 6 extra student events (114 vs 108) but **still scores 100**. ✅
...
- A1 @100ms → B1 @105ms (5ms drift) ✓ Matched
- A2 @200ms → unmatched ✗ (B1 already used, B2 is 50ms away)
- A3 @300ms → unmatched ✗

Result: 1/3 matched = 33% score ❌
```
...
Result: 2/3 matched + time-drift notes = 67% with explanations ✓
```
...
T1 (100ms) ──→ S1 (105ms) ✓     [5ms drift]
T2 (200ms) ──→ S2 (250ms) ✓     [50ms drift]
T3 (300ms) ──→ S3 (350ms) ✓     [50ms drift]
Result: 3/3 = 100%
```
