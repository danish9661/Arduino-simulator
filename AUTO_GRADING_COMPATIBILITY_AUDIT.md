...
### 1.1 **Two Competing Worker Implementations - COLLISION RISK ⚠️**
...
**Problem:**
- **File conflict:** `grading-worker.ts` (v2.2) vs `grading-engine.worker.ts` (v3.8) both exist
- **GradingPage only imports v3.8** (dynamic loader), never uses v2.2 (direct WASM)
- **v2.2 is orphaned:** No UI is calling it, but it's maintained and has different error handling
- **Import statement inconsistency:** v3.8 uses dynamic imports; v2.2 uses static imports
- **Message handler incompatibility:** Slightly different log message formats

**Impact:** 
- If developers accidentally call v2.2 worker, it will fail silently with missing emulator exports
- Maintenance nightmare: Two implementations doing similar work but diverging

**What happens if both workers initialize:**
- v2.2 tries direct imports: `import { FullCircuitValidator } from "@openhw/emulator/..."`
- v3.8 tries dynamic imports: `emulatorExports = await import("@openhw/emulator")`
...
### 1.3 **Emulator Exports Not Guaranteed at Message Time - v3.8 WASM Reference Crash**
...
**Why it crashes:**
- `initEngine()` in v3.8 does: `emulatorExports = await import("@openhw/emulator");`
...
**Fix Needed:** 
1. v3.8 should throw if emulatorExports is missing
2. v2.2 should be the primary implementation
3. Remove v3.8 or properly validate initialization

---

### 1.4 **Polyfill Side-Effects Breaking Worker Context - v3.8**
...
### 4.1 **v2.2 vs v3.8 Response Format Drift**
...
**Issues:**
1. v3.8 sends TWO different message types (`REPORT` and `GRADING_COMPLETE`)
2. GradingPage.jsx only listens for `GRADING_COMPLETE` (line 44)
3. `REPORT` message is lost/ignored
4. v3.8 sends extra data in `finalResult` that v2.2 doesn't:
   ```typescript
...
if (e.data.type === 'GRADING_COMPLETE') {
    if (e.data.result.logs) {  // ← v2.2 has result.logs?
...
**Compatibility Failure:** v2.2 result structure doesn't have `.logs` array
...
**Same format**, but v3.8 might send additional data in future updates while v2.2 doesn't.
...
3. AI worker references `idMapping` that might not exist in v2.2 response
...
**This is good** - case insensitive ✓

**But issue:** What if Windows file system has `.INO` (uppercase)?
- `fileExt()` converts to `.ino` ✓
- File is included ✓
- Arduino compiler might have issues with uppercase extensions on Linux ⚠️
...
| Missing emulatorExports | CRITICAL | v3.8 worker | TypeError when grading |
...
1. **Remove v2.2 or make v3.8 compatible**
...
├── src/worker/
│   ├── grading-worker.ts                    # v2.2 (Primary)
│   ├── grading-engine.worker.ts             # v3.8 (Dynamic - PROBLEM)
