...
export class NotGateLogic extends BaseComponent {
    ...
    update(time: number, wires: any[], instances: BaseComponent[]) {
        ...
        const vIn = this.getPinVoltage('IN');
        const inputHigh = vIn >= 2.5;
        ...
        // Actively drive the output voltage to downstream components through wires
        ...
        const propagate = (pinKey: string, voltage: number) => {
            for (const w of wires) {
                ...
                if (!inst.pins[compPin]) {
                    inst.pins[compPin] = { voltage: 0, mode: 'INPUT' };
                }
                inst.setPinVoltage(compPin, voltage);

                // Traverse through resistors (pass-through)
