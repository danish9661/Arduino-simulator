...
export class NorGateLogic extends BaseComponent {
    constructor(id: string, manifest: any) {
        super(id, manifest);
        this.state = { in1High: false, in2High: false, outputHigh: true };
    }

    update(time: number, wires: any[], instances: BaseComponent[]) {
        super.update(time, wires, instances);

        const in1High = this.getPinVoltage('IN1') >= 2.5;
        const in2High = this.getPinVoltage('IN2') >= 2.5;
        const outputHigh = !(in1High || in2High);

        if (this.state.in1High !== in1High || this.state.in2High !== in2High || this.state.outputHigh !== outputHigh) {
            this.state.in1High = in1High;
            this.state.in2High = in2High;
            this.state.outputHigh = outputHigh;
            this.stateChanged = true;
        }

        // Drive output voltage to downstream components
