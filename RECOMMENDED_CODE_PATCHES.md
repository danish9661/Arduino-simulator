...
if (studentCompileUnit.boardMismatch) {
    postMessage({
        type: 'LOG',
        msg: `⚠️  WARNING: Student board "${boardCompId}" not found. ` +
...
**Reason:**
- Orphaned v2.2 implementation creates confusion
- v3.8 (grading-engine.worker.ts) is the active version
- Having two similar files increases maintenance burden
- Risk of developers accidentally using wrong worker

**If v2.2 features are needed:**
- Merge them into v3.8
