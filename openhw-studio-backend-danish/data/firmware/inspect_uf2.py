...
def main():
    uf2_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path('data/firmware/pico-micropython-uart0.uf2')
    
    if not uf2_path.is_file():
        print(
            f"error: UF2 file not found: {uf2_path}\n"
            f"usage: python {Path(__file__).name} [path/to/firmware.uf2]",
            file=sys.stderr,
        )
        sys.exit(1)

    p = uf2_path.read_bytes()
    vals = []
    ...
    if not vals:
        print(f"error: No valid UF2 blocks found in {uf2_path}", file=sys.stderr)
        sys.exit(1)
