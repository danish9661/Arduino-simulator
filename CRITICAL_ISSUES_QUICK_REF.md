...
- **v2.2 `grading-worker.ts`** - Direct WASM imports (orphaned)
- **v3.8 `grading-engine.worker.ts`** - Dynamic loader with polyfills (ACTIVE)
...
### 10. Message Protocol Drift (v2.2 vs v3.8)
- v3.8 sends extra data in response
...
├── grading-worker.ts (v2.2) - orphaned, direct imports
└── grading-engine.worker.ts (v3.8) - active, dynamic polyfills

UI COORDINATOR:
└── GradingPage.jsx - calls v3.8 worker, spawns AI worker
...
// Keep ONLY grading-engine.worker.ts (v3.8)
